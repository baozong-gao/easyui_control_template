...
<body>
<div class="toolbar">
	<h3 class="tit">修改角色</h3>
	<form:form action="role/update" modelAttribute="rout" method="post" id="roleform">
	<form:hidden path="id"/>
	<div class="list clearfix">
		<div class="item clearfix">
			<lable class="name">角色姓名：</lable>
			<form:input path="name" placeholder="请输入角色姓名"/>
		</div>
		...
		<div class="item clearfix">
			<lable>备注：</lable>
			<form:input path="remark" placeholder="请输入备注"/>
		</div>
							...
							<ul class="selected">
								<c:forEach var="grdrole" items="${subrole.subFunc}">
									<li><input onclick="selectThreeMenu('${role.id}',this)" id="checkItem${grdrole.id}"
											   value="${grdrole.id}"
											   type="checkbox" ${grdrole.auth?"checked":""}/><label
											for="checkItem${grdrole.id}">${grdrole.name}</label></li>
								</c:forEach>
							</ul>
						</div>
					</c:forEach>
	...
	<div>
		<a class="commitForm" onclick="pushRole()">确认修改</a>
	</div>
